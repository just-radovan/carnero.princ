...
public class KulovyParser {

	private static Pattern sTablePattern = Pattern.compile("<div id=\"dnesnacepu\"[^>]*>(.*?)</div>", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
	private static Pattern sBeersPattern = Pattern.compile("<p[^>]*>(.*?)<span[^>]*>[^<]*</span>[^<]*</p>", Pattern.CASE_INSENSITIVE);
	...
	public static ArrayList<Beer> parse(Def definition, String data) {
		...
		while (matcher.find()) {
			...
			for (Pair<String, String> item : filtered) {
				beer = new Beer();
				beer.pub = Constants.LIST_KULOVY.id;
				beer.current = true;
